 ...
 *
 * 저장·화면과 분리한 순수 로직이라 단위 테스트로 고정한다. 목록은 저장소가 아니라 앱이 갖는다 —
...
object Categories {
     ...
     *
     * [CategoryClassifier] 의 키워드 규칙은 이 이름들을 그대로 가리킨다 — 사용자가 설정에서
     * 이름을 바꾸면 그 칩은 더 이상 자동 분류 대상이 아니게 된다(수동으로만 고를 수 있다).
     *
     * «간식»·«육아» 를 기본에 넣은 이유는 그 둘이 없으면 자동 분류가 닿지 못하는 지출이 크기
     * 때문이다. 기저귀·이유식·물티슈는 한 달에 십만 원 단위로 나가는데 «생활» 에 섞이면
     * 그 크기가 보이지 않는다. 안 쓰는 사람에게는 칩 하나가 남을 뿐이다.
     *
     * 이미 쓰고 있는 사람의 목록은 [CategoryStore] 에 저장돼 있어 이 값이 바뀌어도 그대로다 —
     * 새로 설치하는 사람에게만 적용된다.
     */
    val DEFAULT: List<String> = listOf(
        "식비", "카페", "간식", "마트", "교통", "생활", "건강", "육아", "문화", "패션", "주거", "기타",
    )
