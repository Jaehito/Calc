...
2. 속성 이름 3개 확인
3. **개인 곳간** → DB ID와 월 예산 (`930000` 도 `93만` 도 됩니다)
4. **공용 곳간** → 아직 공용 계좌가 없으면 **비워 두세요**
5. **저장하고 연결 확인** → 연결된 곳간마다 DB 제목이 뜨면 성공
6. **알림 켜기** → 알림 권한 허용
...
월 예산을 그 달의 일수로 나눈 값이 **하루치**입니다. 하루치를 다 안 쓰면 남은 만큼이 **곳간**에 쌓이고, 다음 날 쓸 수 있는 돈이 그만큼 늘어납니다. 절약의 보상이 월말이 아니라 다음 날 아침에 옵니다.
...
곳간이 음수가 되면
    모자란 만큼만 그 달 남은 날에 나눠 하루치를 낮춘다
    곳간은 0 으로
```

곳간이 먼저 흡수하기 때문에 크게 쓴 다음 날에도 "오늘 쓸 수 있는 돈"이 음수로 남지 않습니다. 마이너스 잔고는 계획이 아니라 판결문이라 앱을 안 열게 만듭니다.

달이 바뀌면 하루치가 원래대로 돌아가고, 곳간은 **하루치 5일분까지만** 다음 달로 넘어갑니다. 무한히 쌓여 예산이 무의미해지는 것과, 월말에 "어차피 사라지니 쓰자"가 되는 것을 둘 다 막습니다.
